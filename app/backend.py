...
from tables import GRefinement
from tables import Extraction
from tables import Fetch
...
def filters_nerd(user_input):
  ...
  return {'nerd_list': [
             {'name': 'NERD', 'value': it.id} for it in n
            ]}
...
def filters_triplegen(user_input):
  ...
  return {'triplegen_list': [
             {'name': 'TRIPLEGEN', 'value': it.id} for it in tg
            ]}
...
def read_frefinement(user_input):
  print(type(user_input['selected_frefinement']))
...
def create_extraction_url(user_input):
  ...
  filename = f'data/fetch/{getuuid()}.ttl'
...
def create_nrefinement(user_input):
  ...
  selected_nrefinement = NRefienement.query.filter_by(is_none=True, nerd=selected_nerd).first().filename
  filename=f'data/nerd/{getuuid()}.ttl'
  nerd.Nerd(filename=selected_nrefinement).importJSON(nerd_refinement).asGraph().save(filename)
  nr = NRefienement(nerd=selected_nerd, name=name, filename=filename, is_none=False)
  db.session.add(nr)
  db.session.commit()
  return {}
