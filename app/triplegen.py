...
"""
class Alpha:
  def generate(nerdStage, groundTruth):
    pass
...
class Gamma:
  def generate(nerdStage, groundTruth):
    ...
    for ul in nerdStage.mainSection.lists():
      ...
      if individuals == []:
        continue
      statements = [('<http://dbpedia.org/resource/'+subject+'>', '<http://www.w3.org/2004/02/skos/core#related>', '<http://dbpedia.org/resource/'+target_concept+'>')]
      for individual in individuals:
        statements = statements + [('<http://dbpedia.org/resource/'+target_concept+'>', '<http://www.w3.org/2004/02/skos/core#narrower>', '<'+individual.uri+'>')]
        statements = statements + [('<'+individual.uri+'>', '<http://www.w3.org/2000/01/rdf-schema#label>', individual.surface)]
      evidence = subject + ': ' + target_concept + '\n'
      for text in ul.texts():
